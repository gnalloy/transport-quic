# ADR-001: Keep transport-quic as an Independent Gnalloy Module
...
`transport-quic` is published as `gnalloy.org/transport-quic` and owns this responsibility: RFC 9000 QUIC transport for Gnalloy backed by quic-go, plus stream and datagram application exchangers.
