# ADR-001：保持 transport-quic 为独立 Gnalloy 模块
...
`transport-quic` 作为 `gnalloy.org/transport-quic` 发布，并拥有该职责：基于 quic-go 的 Gnalloy RFC 9000 QUIC 传输，以及 stream/datagram 应用 exchanger。
